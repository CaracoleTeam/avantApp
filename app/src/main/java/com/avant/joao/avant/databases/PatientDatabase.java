...
@Database(entities = {PatientEntity.class,Gait.class},version = 1)
    ...
    public static PatientDatabase getDatabase(final Context context){
        if(INSTANCE == null){
            synchronized (PatientDatabase.class){
                if(INSTANCE == null){
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(),PatientDatabase.class,"patient_database").build();
                }
            }
        }
        return INSTANCE;
